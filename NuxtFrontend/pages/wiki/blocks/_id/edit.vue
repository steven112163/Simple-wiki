    ...
    export default {
        ...
        async asyncData({$axios, params}) {
            try {
                ...
                return {
                    block: block,
                    originalBlock: Object.assign({}, block),
                    preview: false,
                    error: null,
                    show: true,
        ...
        methods: {
            ...
            async onSubmit(event) {
                try {
                    ...
                    // Delete image from block object if image wasn't changed
                    if (this.block.image.indexOf("http://") != -1 && !this.newImage)
                        delete this.block["image"];
                    else
                        this.block.image = this.preview;

                    // Create new mob
                    await this.$axios.$patch(`/blocks/${this.block.id}/`, this.block);
            ...
            onReset(event) {
                for (let key in this.block)
                    this.block[key] = this.originalBlock[key];
                this.preview = false;
                this.error = null;
                this.newImage = null;
                this.show = false;
